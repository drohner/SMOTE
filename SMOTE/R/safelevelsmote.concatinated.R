safelevelsmote.concatinated <-
function(data, col, char, k = 5, method = "e", seed = 321, cores = 2){
  #Check input
  .checkinput_sl_con(data, col, char)
  ...
  smotepoints = .safelevelsmote(data, nrow(minority), k,  method, seed, cores)
...
#Checks the given input on validity
.checkinput_sl_con = function(data, col, char){
  if(!(char %in% data[,col])){
